...
# --- Load Angola shapefiles ---
angola <- ne_countries(scale = "medium", country = "Angola", returnclass = "sf")
Ang <- getShp(country = "Angola", admin_level = "admin1")   # provinces

# --- Capital city: Luanda ---
capital_city <- data.frame(name = "Luanda", lon = 13.2894, lat = -8.8383)
capital_city_sf <- st_as_sf(capital_city, coords = c("lon", "lat"), crs = 4326)

# --- Province -> Zone mapping (edit if your province names differ) ---
zone_lut <- tibble::tribble(
  ~name_1,          ~zone,
  "Cabinda",        "North",
  "Zaire",          "North",
  "Uige",           "North",
  "Bengo",          "North",
  "Luanda",         "North",
  "Kwanza Norte",   "North",    # check spelling, sometimes "Cuanza Norte"
  "Malanje",        "North",
  "Lunda Norte",    "East",
  "Lunda Sul",      "East",
  "Moxico",         "East",
  "Kwanza Sul",     "Central",  # check spelling, sometimes "Cuanza Sul"
  "Bie",            "Central",
  "Huambo",         "Central",
  "Benguela",       "Central",
  "Huila",          "South",
  "Cunene",         "South",
  "Namibe",         "South",
  "Kuando Kubango", "South"     # check spelling, sometimes "Cuando Cubango"
)

Ang <- Ang %>%
  left_join(zone_lut, by = "name_1")

# --- Label points (for province names inside polygons) ---
label_points <- st_point_on_surface(Ang)

# --- Plot ---
ggplot(data = Ang) +
  geom_sf(data = angola, fill = "white", color = "black", size = 0.5) +
  geom_sf(aes(fill = zone), color = "black") +   # <-- use your new zone variable
  geom_sf_text(aes(label = ifelse(name_1 == "Luanda", "", name_1)), 
               size = 3, color = "black") +      # <-- suppress Luanda province label
  geom_sf(data = capital_city_sf, color = "blue", size = 3, shape = 16) +
  annotate("text", x = 13.2894, y = -8.8383, label = "Luanda", 
           hjust = -0.1, color = "blue", fontface = "bold", size = 4) +
  coord_sf() +
  scale_fill_brewer(palette = "Set3") +
  theme_minimal() +
...
The country has a high fertility rate, it is the second highest in Africa and one of the highest in the world with around 6 children per woman (2018) [National Institute of Statistics of Angola]. This impacts on malaria control due to high demand for antenatal services.
...
names(angola_pop) <- c("Age_group", "2021", "2022", "2023", "2024", "2025")
